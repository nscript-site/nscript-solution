...
using Avalonia.NScript.Models;
using Avalonia.Styling;

namespace Avalonia.NScript.Controls;

public partial class NScriptTheme : Styles
{
    public static readonly StyledProperty<SukiColor> ThemeColorProperty =
        AvaloniaProperty.Register<NScriptTheme, SukiColor>(nameof(Color), defaultBindingMode: BindingMode.OneTime,
            defaultValue: SukiColor.Blue);

    public static readonly StyledProperty<bool> IsRightToLeftProperty =
        AvaloniaProperty.Register<NScriptTheme, bool>(nameof(IsRightToLeft), defaultBindingMode: BindingMode.OneTime,
            defaultValue: false);
    ...
    public static NScriptTheme GetInstance(Application app)
    {
        var theme = app.Styles.FirstOrDefault(style => style is NScriptTheme);
        if (theme is not NScriptTheme sukiTheme)
    ...
    /// <returns>A <see cref="SukiTheme"/> instance that can be used to change themes.</returns>
    /// <exception cref="InvalidOperationException">Thrown if no SukiTheme has been defined in App.axaml.</exception>
    public static NScriptTheme GetInstance() => GetInstance(Application.Current!);

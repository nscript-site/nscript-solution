...
/// <summary>
/// 每一个 TEntity 存储为独立的 litedb 数据库文件。
/// </summary>
/// <typeparam name="TEntity"></typeparam>
public class MultiFileDataService<TEntity> : BaseDataService where TEntity : class
{
    protected String CollectionName { get; set; } = "default";

    protected readonly string _databaseName;

    public MultiFileDataService(string? databaseName = null, string? baseDir = null)
    {
    ...
    {
        return String.IsNullOrEmpty(_databaseName)?String.Empty: _databaseName;
    }
            ...
            if (_dbName == null)
            {
                ...
                if(String.IsNullOrEmpty(bucketName) == false)
                {
                    if (dbName.EndsWith(".db"))
                        dbName = dbName.Substring(0, dbName.Length - 3) + "#" + _databaseName + ".db";
                    else
